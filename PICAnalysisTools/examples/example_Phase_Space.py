...
E_Phase, plt_limits, Z_Bins, E_Bins, z_line, Energy_line = Phase_Space.Energy_z_space(z_res=0.1, z_round=5, Spec_Res=0.2, E_Round=50, Centre_z=True, lineout_height=0.2)
...
plt.plot(Z_Bins, z_line, color = 'blue', linewidth = 0.5)
...
Et_Phase, plt_limits, T_Bins, E_Bins, t_line, Energy_line = Phase_Space.Energy_time_space(t_res=0.1, t_round=10, Spec_Res=0.2, E_Round=50, Centre_t = True, lineout_height=0.2)

fig1, ax1 = plt.subplots()
plt.imshow(Et_Phase, cmap=cmap_white(plt.cm.inferno), extent=(plt_limits), aspect='auto', norm=LogNorm())
plt.colorbar().set_label(label='Number of electrons',size=fsize)
plt.clim(plt_limits_log(Et_Phase, min_offset = 0, max_offset = 1))

plt.plot(Energy_line, E_Bins, color = 'forestgreen', linewidth = 0.5)
plt.plot(T_Bins, t_line, color = 'forestgreen', linewidth = 0.5)
...
Trans_phase = Phase_Space.Div_r_space('x', r_res=0.20, r_round=10, div_res=0.1, div_round=10, lineout_height=0.2)

fig1, ax1 = plt.subplots()
plt.imshow(Trans_phase[0], cmap=cmap_white(plt.cm.viridis), extent=(Trans_phase[1]), aspect='auto', norm=LogNorm())
plt.colorbar().set_label(label='Number of electrons',size=fsize)
plt.clim(plt_limits_log(Trans_phase[0], min_offset = 0, max_offset = 1))

plt.plot(Trans_phase[2], Trans_phase[4], color = 'red', linewidth = 1)
plt.plot(Trans_phase[5], Trans_phase[3], color = 'red', linewidth = 1)

ax1.set_title("c$\\tau$ = %0.2f mm" % round(ctau,2), fontsize=fsize)
ax1.set_xlabel('x ($\\mu$m)', fontsize=fsize)
ax1.set_ylabel('x\' (mrad)', fontsize=fsize)
plt.axis(Trans_phase[1])
plt.show()
...
fig1, ax1 = plt.subplots()
plt.imshow(YAG, cmap=cmap_white(plt.cm.winter), extent=(Hist_Limits), aspect='auto', norm=LogNorm())
plt.colorbar().set_label(label='Number of electrons',size=fsize)
...
plt.axis(Hist_Limits)
plt.show()


#%% Test beam projection class - xz space projection

BP_xz = BeamProjection(z, x, w) # order should be x-axis, y-axis

xz_project, hist_limits, _, _ = BP_xz.beam_projection(r_res=0.2, r_round=10, independant_bins=True, equal_bins=False)

fig1, ax1 = plt.subplots()
plt.imshow(xz_project, cmap=cmap_white(plt.cm.spring), extent=(hist_limits), aspect='auto', norm=LogNorm())
plt.colorbar().set_label(label='Number of electrons',size=fsize)
...
fig1, ax1 = plt.subplots()
plt.imshow(xz_project_full, cmap=cmap_white(plt.cm.spring), extent=([window_limits[2], window_limits[3], window_limits[0], window_limits[1]]), aspect='auto', norm=LogNorm())
plt.colorbar().set_label(label='Number of electrons',size=fsize)
...
fig1, ax1 = plt.subplots()
plt.imshow(xz_project, cmap=cmap_white(plt.cm.plasma), extent=(hist_limits), aspect='auto', norm=LogNorm())
plt.colorbar().set_label(label='Number of electrons',size=fsize)
plt.clim(plt_limits_log(xz_project, min_offset = 0, max_offset = 1))
